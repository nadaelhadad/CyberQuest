...
import { Lock, Download, FileArchive, Search, Wrench, ArrowLeft, Trophy, Clock, Sparkles, Star, CheckCircle } from 'lucide-react';
import Button from './ui/Button';
import Input from './ui/Input';
import { useGameStore } from '../store/gameStore';

const WeakZipChallenge = () => {
  const navigate = useNavigate();
  const { categories, submitFlag } = useGameStore();
  const challenge = categories
    .find(cat => cat.id === 'reversing')
    ?.challenges.find(ch => ch.id === 'reversing-2');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (submitFlag('reversing-2', flag)) {
      setSuccess(true);
    } else {
  ...
  const handleDownload = () => {
    ...
    link.href = '/challenges/encrypted_logs.zip';
  ...
  return (
    <div className="min-h-screen bg-cyber-black text-white p-6" onContextMenu={(e) => e.preventDefault()}>
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-8 bg-cyber-black/50 border border-cyber-blue-500/30 rounded-lg p-4 h-fit">
            ...
            <div className="text-cyber-gray-300 mb-4">
              ...
              <p className="text-sm text-cyber-purple-300 italic">Use forensic ZIP tools and GUI-based password recovery to access the file.</p>
            </div>
          ...
          {/* Right Panel - Investigation Tools */}
          <div className="col-span-4 space-y-6">
            {/* Archive Comment Card */}
            <div className="bg-cyber-black/50 border border-cyber-blue-500/30 rounded-lg p-6">
              <h3 className="text-lg font-semibold text-cyber-blue-400 mb-4 flex items-center gap-2">
                <Search className="w-5 h-5" />
                Inspect Archive Comment
              </h3>
              
              <Button
                onClick={handleDownload}
                className="w-full bg-gradient-to-r from-emerald-800 to-green-700 hover:from-emerald-700 hover:to-green-600 text-white px-6 py-3 rounded-lg transition-all duration-300 transform hover:scale-105 hover:shadow-lg hover:shadow-emerald-800/40 font-semibold flex items-center gap-2"
              >
              ...
              <Button
                onClick={() => window.open('https://www.openwall.com/john/k/john-1.9.0-jumbo-1-win64.zip', '_blank')}
                className="w-full mt-4 bg-gradient-to-r from-emerald-800 to-green-700 hover:from-emerald-700 hover:to-green-600 text-white px-6 py-3 rounded-lg transition-all duration-300 transform hover:scale-105 hover:shadow-lg hover:shadow-emerald-800/40 font-semibold flex items-center gap-2"
              >
                <Download className="w-4 h-4" />
                <span> Download John the Ripper</span>
              </Button>
            </div>

            {/* CyberChef Card */}
            <div className="bg-cyber-black/50 border border-cyber-pink-500/30 rounded-lg p-6">
              <h3 className="text-lg font-semibold text-cyber-pink-400 mb-4 flex items-center gap-2">
                <Wrench className="w-5 h-5" />
                CyberChef (Decrypt Tools)
              </h3>
              <Button
                onClick={() => window.open('https://gchq.github.io/CyberChef/', '_blank')}
                className="w-full bg-gradient-to-r from-pink-600 to-rose-500 hover:from-pink-500 hover:to-rose-400 text-white px-6 py-3 rounded-lg transition-all duration-300 transform hover:scale-105 hover:shadow-lg hover:shadow-pink-600/40 font-semibold flex items-center gap-2"
              >
                <span>🧪 Open CyberChef</span>
              </Button>
...
export default WeakZipChallenge; 