        ...
        <div className="lg:col-span-9 lg:col-start-2 xl:col-span-9 xl:col-start-2 flex flex-col justify-center">
          ...
          >
            <Card className="p-12 md:p-16 xl:p-20 bg-cyber-black bg-opacity-80 border border-cyber-blue-200 text-lg xl:text-xl min-h-[500px] flex flex-col justify-center">
              <div className="mb-6">
                <h1 className="text-2xl font-cyber text-cyber-blue-100 mb-2">🪞 Mirror Block – Gold</h1>
                <div className="flex items-center gap-4">
                  <p className="text-white text-lg flex-1">A 160-byte file shows repeating blocks — classic ECB. Our mole stole the key "RED_DAWN_SECRET0". Break strike three.</p>
                  <Button
                    onClick={() => {
                      const link = document.createElement('a');
                      link.href = '/challenges/whisper3.bin';
                      link.download = 'whisper3.bin';
              ...
              {/* Key Display Section */}
              <div className="mb-6">
                <h2 className="text-cyber-yellow text-sm font-bold mb-2">Decryption Key</h2>
                <div className="flex items-center gap-3 p-3 bg-gradient-to-r from-cyan-600/10 to-blue-600/10 border border-cyber-blue-200/20 rounded-lg">
                  <span className="text-cyber-blue-100 font-mono text-sm">RED_DAWN_SECRET0</span>
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText('RED_DAWN_SECRET0');
                      setSubmissionStatus('success');
                      setSubmissionMessage('✅ Key copied to clipboard!');
                      setTimeout(() => {
                        setSubmissionStatus('idle');
                        setSubmissionMessage('');
                      }, 2000);
                    }}
                    className="flex items-center justify-center p-2 bg-gradient-to-r from-cyan-600/20 to-blue-600/20 hover:from-cyan-500/20 hover:to-blue-500/20 rounded transition-all duration-300 hover:scale-105"
                    title="Copy key to clipboard"
                  >
                    <Copy size={16} className="text-cyber-blue-100" />
                  </button>
                </div>
              </div>
